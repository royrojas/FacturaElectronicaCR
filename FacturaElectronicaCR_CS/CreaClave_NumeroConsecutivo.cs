...
{
    public partial class CreaClave_NumeroConsecutivo : Form
    {
        public CreaClave_NumeroConsecutivo()
        {
            InitializeComponent();
        }

        private void btnGeneraDatos_Click(object sender, EventArgs e)
        {
            try
            {
                String numeroConsecutivo = CreaNumeroConsecutivo(txtSucursal.Text,
                                                                 txtPOS.Text,
                                                                 txtTipoComprobante.Text,
                                                                 txtNumeroComprobante.Text);

                txtNumeroConsecutivo01.Text = numeroConsecutivo;
                txtNumeroConsecutivo02.Text = numeroConsecutivo;

                txtCodigoSeguridad.Text = CreaCodigoSeguridad(txtTipoComprobante.Text,
                                                              txtSucursal.Text,
                                                              txtPOS.Text,
                                                              dtpFecha.Value,
                                                              txtNumeroComprobante.Text);

                String clave = CreaClave(txtCodigoPais.Text,
                                         dtpFecha.Value.Day.ToString(),
                                         dtpFecha.Value.Month.ToString(),
                                         dtpFecha.Value.Year.ToString(),
                                         txtIdentificacion.Text,
                                         numeroConsecutivo,
                                         txtSituacionComprobante.Text,
